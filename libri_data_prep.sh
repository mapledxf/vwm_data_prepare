...
spk_file=$db/SPEAKERS.TXT

for part in train-other-500 train-clean-360 dev-clean test-clean dev-other test-other train-clean-100; do
  src=$db/$part
  dst=$out/$(echo $part | sed s/-/_/g)
  mkdir -p $dst || exit 1;
